classdef ExperimentManager
    ...
    methods (Access = public, Static = true)
        ...
        function chPath = GetPathToExperimentAssetResultsDirectory(chAssetIdTag)
            ...
            if ~strcmp(which('Experiment'), 'Not on MATLAB path') && Experiment.IsRunning()
                chExperimentRoot = Experiment.GetDataPath('ExperimentRoot');
                voEntries = dir(fullfile(Experiment.GetDataPath('ExperimentRoot'), sToFolderPath));
            else
                chExperimentRoot = pwd;    
                voEntries = dir(fullfile(chExperimentRoot, sToFolderPath));            
            end                        
            ...
            if sum(vbMatches) ~= 1
                if sum(vbMatches) == 0
                    % repeat search in second ExperimentRoot
                    chSecondExperimentRoot = '';
                    
                    try
                        chSecondExperimentRoot = Experiment.GetDataPath('ExperimentRoot_2');
                    catch e
                    end
                    
                    if ~isempty(chSecondExperimentRoot)
                        chExperimentRoot = chSecondExperimentRoot;
                        voEntries = dir(fullfile(chExperimentRoot, sToFolderPath));
